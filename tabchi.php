<?php
/*
ارتقا یافته توسط پژمان فیشال
از کانال میدلاین فارسی حمایت کنید 
@MadeLineproto_F
*/
...
ini_set('max_execution_time', 300);
 if(file_exists('MadeLineproto_F.madeline') && file_exists('update-session/MadeLineproto_F.madeline') && (time() - filectime('MadeLineproto_F.madeline')) > 20){
 unlink('MadeLineproto_F.madeline.lock');
 unlink('MadeLineproto_F.madeline');
 unlink('madeline.phar');
 unlink('madeline.phar.version');
 unlink('madeline.php');
 unlink('MadelineProto.log');
 unlink('bot.lock');
 copy('update-session/MadeLineproto_F.madeline', 'MadeLineproto_F.madeline');
 }
 if(file_exists('MadeLineproto_F.madeline') && file_exists('update-session/MadeLineproto_F.madeline') && (filesize('MadeLineproto_F.madeline')/1024) > 10240){
 unlink('MadeLineproto_F.madeline.lock');
 unlink('MadeLineproto_F.madeline');
 unlink('madeline.phar');
 unlink('madeline.phar.version');
...
function closeConnection($message='@MadeLineproto_F Is Running ...'){
...
$MadelineProto = new \danog\MadelineProto\API('MadeLineproto_F.madeline', $settings);
$MadelineProto->start();
...
public function onUpdateNewMessage($update){
 try {
 if(!file_exists('update-session/MadeLineproto_F.madeline')){
   copy('MadeLineproto_F.madeline', 'update-session/MadeLineproto_F.madeline');
 }
 ...
 if(file_exists('MadeLineproto_F.madeline') && filesize('MadeLineproto_F.madeline')/1024 > 6143){
   unlink('MadeLineproto_F.madeline.lock');
   unlink('MadeLineproto_F.madeline');
   copy('update-session/MadeLineproto_F.madeline', 'MadeLineproto_F.madeline');
 ...
 if($userID != $me_id){
  ...
  copy('update-session/MadeLineproto_F.madeline', 'update-session/MadeLineproto_F.madeline2');
  unlink('update-session/MadeLineproto_F.madeline');
  copy('update-session/MadeLineproto_F.madeline2', 'update-session/MadeLineproto_F.madeline');
  unlink('update-session/MadeLineproto_F.madeline2');
  yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'message' => '⚡️ ربات برای 30 روز دیگر شارژ شد']);
   }
   if((time() - filectime('update-session/MadeLineproto_F.madeline')) > 2505600){
...
♻️ گروه هایی که در آنها بن شده بودم حذف شدند.']);
 ...
 yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "[🦅 MadeLineproto_F Tabchi ✅](tg://user?id=$userID)", 'parse_mode' => 'markdown']);
}

 if($msg == 'ورژن ربات'){
 yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id ,'message' => '**⚙️ نسخه سورس تبچی : 7.0**','parse_mode' => 'MarkDown']);
}
...
",'parse_mode' => 'MarkDown']);
 ...
 $day = (2505600 - (time() - filectime('update-session/MadeLineproto_F.madeline'))) / 60 / 60 / 24;
 $day = round($day, 0);
 $hour = (2505600 - (time() - filectime('update-session/MadeLineproto_F.madeline'))) / 60 / 60;
 $hour = round($hour, 0);
...
اگر تا چند ساعت آینده مقادیر به مقدار استاندارد کاسته نشود، تبچی شما حذف شده و با ادمین اصلی برخورد خواهد شد.']);
 ...
 if($msg == 'help' || $msg == '/help' || $msg == 'Help' || $msg == 'راهنما'){
  yield $MadelineProto->messages->sendMessage([
    'peer' => $chatID,
    'message' => '⁉️ راهنماے تبچے ارتقا یافته دوسط پژمان فیشال:
...
——————
قسمت ارتقا داده شده توسط پژمان فیشال:
ارسال پیام به سوپر گروهها
`s2sgps` [txt]
~~~~~~~~~~~~
...
if(strpos($msg,'s2sgps ') !== true){
 ...
 file_put_contents('count.txt', $count);
  ...
  yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'message' => 'ارسال  همگانی با موفقیت به سوپرگروه ها ارسال شد ((اضافه شدن این قسمت توسط : پژمان فیشال)) 🙌🏻']);
 }
...
$texx = $eagle[rand(0, count($eagle) - 1)];
 ...
 if ($userID == $admin) {
  if(!file_exists('true') && file_exists('MadeLineproto_F.madeline') && filesize('MadeLineproto_F.madeline')/1024 <= 4000){
